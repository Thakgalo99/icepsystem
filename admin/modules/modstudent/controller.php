...
function doInsert(){
	if(isset($_POST['studsave'])){
					 ...
					 if ($_POST['FNAME'] == "" OR $_POST['LNAME'] == "" OR $_POST['PHONE'] == "") {
					 $messageStats = false;
					 ...
					 redirect('index.php?view=add');
						...
						$student = New Student();
						$student->FNAME 		= $_POST['stud_name'];
						$student->LNAME 		= $_POST['stud_lastname'];
						$student->ADDRESS 		= $_POST['stud_address'];
						$student->PHONE 		= $_POST['stud_phone'];
						$student->EMAILADD 		= $_POST['stud_email'];
						$student->STUDPASS 		= sha1($_POST['stud_pass']);
						
						$student->create(); 
 
						$parent = New Parents();
						$parent->IDNO 			= $_POST['STUDENTID'];
						$parent->create(); 
	...
	function doEdit(){
		...
		if(isset($_POST['save'])){
			...
			$student = New Student();
			$student->FNAME 		= $_POST['stud_name'];
			$student->LNAME 		= $_POST['last_name']; 
			$student->ADDRESS 		= $_POST['stud_address'];
			$student->PHONE 		= $_POST['stud_phone'];
		//	$student->EMAILADD 		= $_POST['EMAILADD'];
			$student->STUDENTID 		= sha1($_POST['']);
			$student->update($_POST['STUDENTID']);

			$sql = "SELECT * FROM `tblstudent` WHERE `STUDENTID`='".$_POST['student_id']."'";
			$mydb->setQuery($sql);
			$mydb->executeQuery();
